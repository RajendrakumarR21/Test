...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] = {
      var urlSeq : Seq[Try[URL]] = Seq()
      for(des <- ns.descendant) {
       for(hrAtt <- des.attribute("href")) {
         urlSeq = Seq(createURL(Try(new URL(u.getProtocol.concat(":") )).toOption, hrAtt.toString()))
       }
      }
      //val res =Try{ u}
      //val seqres = Seq(res)
    ...
    //Try(new URL(u.getProtocol + “:”)).toOption
    ...
    MonadOps.asFuture(getLinks(Await.result(getURLContent(u),duration)))  // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED Future[Seq[URL]]
    for (u <- us2) yield {
     val x = MonadOps.sequence(wget(u))   //Future[Seq[URL]]-->  Future[Either[Throwable, Seq[URL]]]
    }
  }
